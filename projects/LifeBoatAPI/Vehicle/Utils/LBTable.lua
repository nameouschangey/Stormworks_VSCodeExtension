...
function LBTable_containsValue(tbl, value)
...
---@return boolean exists whether this value exists in the collection or not
---@section LBTable_containsKey
function LBTable_containsKey(tbl, key)
    return tbl[key] ~= nil
end
---@endsection
...
function LBTable_countKeys(tbl)
    local i = 0
    for _, v in pairs(tbl) do
        i = i + 1
    end
...
function LBTable_remove(tbl, key)
...
---@section LBTable_add
function LBTable_add(tbl, value, index)
...
function LBTable_addRange(tbl, from)
    LBCopy(from, tbl)
end
---@endsection
...
function LBTable_iaddRange(tbl, from)
...
---@section LBTable_flattenValues
function LBTable_flattenValues(tbl)
    local result = {}
...
---@section LBTable_flattenKeys
function LBTable_flattenKeys(tbl)
    local result = {}
    for k,v in pairs(tbl) do
...
function LBTable_ifillHoles(tbl)
    local result = {}
    for k,v in pairs(tbl) do
        if(type(k) == "number") then
            LBTable_add(result, v)
        else
            result[k] = v
        end
    end
...
function LBTable_islice(tbl, startIndex, endIndex)
    local result  = {}
...
function LBTable_where(tbl, callable)
    local result = {}
    for k,v in pairs(tbl) do
...
function LBTable_iwhere(tbl, callable)
    local result = {}
...
function LBTable_select(tbl, callable)
    local result = {}
    for k,v in pairs(tbl) do
...
function LBTable_iselect(tbl, callable)
    local result = {}
    for k,v in ipairs(tbl) do
...
function LBTable_groupBy(tbl, callable)
    local result = {}
    for k,v in pairs(tbl) do
...
function LBTable_igroupBy(tbl, callable)
    local result = {}
    for k,v in ipairs(tbl) do
        local groupableValue = callable(v, k)
        result[groupableValue] = result[groupableValue] or {}
...
function LBTable_iorderBy(tbl, comparison)
    local result = LBCopy(tbl, {}, true)
...
---@section LBTable_concat
function LBTable_concat(tbl, separator)
    ...
    for k,v in pairs(tbl) do
        ...
        if(type(v) == "table" and not seen[v]) then
            ...
            if(not maxDepth or depth < maxDepth) then
                LBTable_recurse(v, callable, maxDepth, depth+1, newPath, seen)
            end
        end
    end
end
